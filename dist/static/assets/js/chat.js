document.addEventListener("DOMContentLoaded", function () {
  ...
  socket.on("receive_message", function (message) {
    if (
      ...
      // Jika pesan masuk dari pengguna lain yang bukan kontak aktif
      // Tambahkan pengirim ke daftar kontak secara dinamis
      addContactDynamically(message.sender);
    }
  });



  function addContactDynamically(username) {
    // Cek apakah kontak sudah ada di daftar
    const existingContact = document.querySelector(
      `.contact-item[data-username="${username}"]`
    );
    if (!existingContact) {
      fetch(`/get_contact_info/${username}`)
        .then((response) => response.json())
        .then((data) => {
          if (data) {
            ...
            contactItem.innerHTML = `
              <div class="d-flex align-items-center">
                <div class="flex-shrink-0 me-3">
                  <span class="avatar avatar-md brround bg-primary-transparent text-primary">${username.charAt(0).toUpperCase()}</span>
                </div>
                <div class="flex-grow-1">
                  <h6 class="mb-0">${username}</h6>
                  <small class="text-muted status-text">${data.status}</small>
                </div>
                <div class="ms-auto">
                  <span class="rounded-circle ${data.status === 'online' ? 'bg-success' : 'bg-danger'}"></span>
                </div>
              </div>
            `;
            document.querySelector(".contacts-list").appendChild(contactItem);

            // Tambahkan event listener untuk kontak baru
